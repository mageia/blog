...
def index(request):
    try:
        ...
        global_data = global_setting(request)
        # 文章归档
        ...
        # 直接执行原生sql呢？
        # 第一种方式（不可行）
        # archive_list =Article.objects.raw('SELECT id, DATE_FORMAT(date_publish, "%%Y-%%m")
        # as col_date FROM blog_article ORDER BY date_publish')
        # for archive in archive_list:
        #     print archive
        # 第二种方式（不推荐）
        # cursor = connection.cursor()
        # cursor.execute("SELECT DISTINCT DATE_FORMAT(date_publish, '%Y-%m')
        # as col_date FROM blog_article ORDER BY date_publish")
        # row = cursor.fetchall()
        # print row
...
def tag(request, tag_name = None):
    try:
        if tag_name:
            ...
            article_list = Article.objects.filter(tag=tags)
            article_list = getPage(request, article_list)
            global_data = global_setting(request)
        return render(request, 'index.html', locals())
    except Exception as e:
